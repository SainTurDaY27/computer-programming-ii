...
class AnalogClock(tk.Frame):
    ...
    def update_clock(self):
        ...
        self.clock_face = self.canvas.create_oval(radius + cx, radius + cy, cx - radius, cy - radius, width=3, outline="blue")

        # update the item self.center_pin so that it has the radius of 5 pixels and
        # is centered at coordinates (cx,cy)
        self.center_pin = self.canvas.create_oval(cx + 2.5, cy + 2.5, cx - 2.5, cy - 2.5, fill="green")

        now = datetime.now()
        # second_operation
        sec_angle = now.second / 60 * 360
        x_sec, y_sec = self.polar_to_canvas(radius * 0.9, sec_angle)
        self.canvas.coords(self.second_hand, cx, cy, x_sec, y_sec)

        # min_operation
        min_angle = now.minute / 60 * 360 + now.second / 10
        x_min, y_min = self.polar_to_canvas(radius * 0.8, min_angle)
        self.canvas.coords(self.minute_hand, cx, cy, x_min, y_min)

        # hour_operation
        if now.hour < 13:
            hour_angle = ((now.hour / 12) * 360) + now.minute / 6
        else:
            hour_angle = (((now.hour - 12) / 12) * 360) + now.minute / 6
        x_hour, y_hour = self.polar_to_canvas(radius * 0.5, hour_angle)
        self.canvas.coords(self.hour_hand, cx, cy, x_hour, y_hour)
